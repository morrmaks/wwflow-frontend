import type { RevokeCanvasInviteMutationVariables } from '@src/common/api/graphql/__generated__';
...
const revokeCanvasInvite: GraphQLRequestConfig = {
  operationType: 'mutation',
  identifier: 'RevokeCanvasInvite',
  interceptors: {
  ...
  routes: [
    {
      // data: (request) => {
      data: graphqlData<RevokeCanvasInviteMutationVariables>(({ request }) => {
        const error = requireAuth(request);
        if (error) return error;

        const { inviteId } = request.body.variables;

        const result = CanvasPermissions.revokeInvite(
          inviteId,
          request.user?.id || '',
          request.context.orm
        );
        ...
        if (result.success) {
          return {
            data: {
              revokeCanvasInvite: result.data
            }
          };
        }
