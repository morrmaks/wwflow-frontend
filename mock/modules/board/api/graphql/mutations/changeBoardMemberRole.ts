import type { ChangeBoardMemberRoleMutationVariables } from '@src/common/api/graphql/__generated__';
...
const changeBoardMemberRole: GraphQLRequestConfig = {
  operationType: 'mutation',
  identifier: 'ChangeBoardMemberRole',
  interceptors: {
  ...
  routes: [
    {
      data: graphqlData<ChangeBoardMemberRoleMutationVariables>(({ request }) => {
        ...
        const result = BoardPermissions.changeRole(
          memberId,
          role,
          request.user?.id || '',
          request.context.orm
        ...
        if (result.success) {
          return {
            data: {
              changeBoardMemberRole: result.data
            }
          };
        }
