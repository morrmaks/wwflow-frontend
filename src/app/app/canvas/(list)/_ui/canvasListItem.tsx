...
import { CanvasListItemDropdownMenu } from './canvasListItemDropdownMenu';

interface CanvasListItemProps {
  canvas: CanvasListFieldsFragment;
}

function CanvasListItem({ canvas }: CanvasListItemProps) {
  const { updatedAt, id, previewUrl, title, membersCount, myRole } = canvas;
    ...
    <Card className='transition-colors hover:border-primary pt-0 overflow-hidden'>
      ...
      <CardFooter className='flex flex-col gap-3'>
        <div className='flex items-center justify-between w-full gap-2'>
          ...
          <CanvasListItemDropdownMenu canvas={canvas} />
        </div>
